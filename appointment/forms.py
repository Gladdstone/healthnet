    ...
    def is_valid(self, **kwargs):
        ...
        try:
            ...
            if (len(i) > 0 and (this==None or title != this.title)):
                ...
                return False
            d = user.patient_appointment.filter(month= date_month,
                                    year = date_year,
                                    day = date_day,
                                    time = time)
            if (len(d) > 0 and (this==None or date_month != this.month or date_year != this.year or date_day != this.day or time != this.time)):
